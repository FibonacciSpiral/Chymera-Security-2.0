...
unsigned long _new_course;
unsigned long _hdop = GPS_INVALID_HDOP;
unsigned long _new_hdop;
unsigned short _numsats = GPS_INVALID_SATELLITES;
unsigned short _new_numsats;
unsigned long _last_time_fix = GPS_INVALID_FIX_TIME;
unsigned long _new_time_fix;
unsigned long _last_position_fix = GPS_INVALID_FIX_TIME;
unsigned long _new_position_fix;
uint8_t _parity = 0;
int _is_checksum_term = 0;
uint8_t _sentence_type = _GPS_SENTENCE_OTHER;
uint8_t _term_number = 0;
uint8_t _term_offset = 0;
int _gps_data_good = 0;
#ifndef _GPS_NO_STATS
unsigned long _encoded_characters = 0;
unsigned short _good_sentences = 0;
unsigned short _failed_checksum = 0;
#endif
char _term[15] = {'\0'}; //made change here
...
float radians(float degrees) //returns the radians from a degree measure
{
...
void print_float(float val, int len, char* buffer)
{
    snprintf(buffer, len, "%f", val);
}
