...
const int alarmLength = 90;
float GPS_SEND_FREQ_SEC = 1.2;
int contTrackOnAlarmEnable = 1;
int gpsEnable = 1;
int enteringDataMode = 0;
int callEnable = 1;
uint8_t myDeviceName[2] = {'\0'};
char myPhoneNumber[20] = "ATD5412952229;\r\n";
    ...
    while(1)
    {
        while(arm_flag == 1)
        {
            ...
            if(panicFlag == 1)
            {
                ...
                if (alarmOn == 0)
                {
                    if (contTrackOnAlarmEnable==1)
                    {
                        //change GPS Frequency
                        GPS_SEND_FREQ_MS = (int) ceil(1.2*1000.0);
                    }
                    alarmOn = 1;
        ...
        while ((arm_flag == 0) && (data_on == 0)) //if disarmed and data is not on
        {
            if (once == 1)
            {
                ...
                {
                    //change GPS Frequency
                    GPS_SEND_FREQ_MS = (int) ceil(GPS_SEND_FREQ_SEC*1000.0); //set it back to whatever it was
                }
                once = 0;
                delay(250);
                pingCount = 0; //set this to 0 to initialize
